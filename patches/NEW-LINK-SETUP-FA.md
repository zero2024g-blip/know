...
- **بدون رشته‌ی واقعی در باینری:** همه‌ی رشته‌های حساس با `OBF("...")` هستند — در باینری
  بایتِ آشغال، در سورس خوانا، زمانِ اجرا رمزگشایی. **تست‌شده:** `strings`/گرپِ باینری هیچ‌کدام
  از `EagleA/1.2`، `user_key`، `zezr_connector`، `YOUR_STATIC_WORDS` را نشان نمی‌دهد (با g++ و clang).
