...
2. اسم کپی را به **`.env`** تغییر بده — نقطه در **اولِ** اسم، و بعد از
   `env` هیچی نیاید. `env.` (نقطه در آخر) غلط است و خوانده نمی‌شود.
...
**می‌روم توی پنل و پرت می‌شوم به `localhost:8080`**
**«localhost refused to connect» می‌بینم**
**صفحه بی‌ریخت است و همهٔ CSS از بین رفته**

هر سه یک علت دارند: **فایل `.env` خوانده نمی‌شود.** بدون آن پنل به مقدار
پیش‌فرض `Config/App.php` برمی‌گردد که `http://localhost:8080/` است، پس هر
لینک و هر فایل CSS به `localhost` اشاره می‌کند. صفحه‌ها بالا می‌آیند، پس
به‌نظر می‌رسد پنل کار می‌کند — ولی هیچ‌جا اسم `.env` نمی‌آید.

از این نسخه به بعد پنل خودش این را می‌گوید: به‌جای صفحهٔ خراب، یک صفحهٔ
قرمز می‌بینی که **دقیقاً مسیری را می‌نویسد که دنبال فایل می‌گردد**.
سه علت رایج:

1. `.env` اصلاً ساخته نشده — `env.template` را کپی و rename نکرده‌ای
2. **اسمش غلط است** — این رایج‌ترین است. نقطه باید **اولِ** اسم باشد:
   `.env` درست است؛ `env.` و `env` و `.env.txt` هیچ‌کدام خوانده نمی‌شوند.
   پنل حالا خودش این را تشخیص می‌دهد و اسم فایلی که داری را می‌نویسد.
3. جای اشتباه است — باید **کنار `index.php`** باشد، **نه** داخل `public/`
