...
## هاست شما: Hostinger + LiteSpeed

این‌ها را از پاسخ زنده‌ی سایت خواندم، نه حدس:

```
x-turbo-charged-by: LiteSpeed      ← وب‌سرور LiteSpeed است، نه Apache
platform: hostinger / panel: hpanel
server: cloudflare                  ← الان MODE A فعال است
content-encoding: br                ← فشرده‌سازی کار می‌کند (از سمت Cloudflare)
```

### چه چیزی به‌خاطر LiteSpeed فرق می‌کند

LiteSpeed فایل `.htaccess` را می‌خواند، ولی **نام ماژول‌های Apache را
گزارش نمی‌کند**. یعنی بلوک‌های `<IfModule mod_deflate.c>` و
`<IfModule mod_brotli.c>` روی هاست شما به احتمال زیاد **اصلاً اجرا
نمی‌شوند**.

این مشکلی ایجاد نمی‌کند، چون:

- در MODE A، Cloudflare خودش فشرده می‌کند (تأیید شد)
- در MODE B، فشرده‌سازی LiteSpeed از **hPanel** روشن می‌شود، نه از
  `.htaccess` — مسیر: hPanel ← Website ← Optimization
- سرو فایل‌های از پیش فشرده‌ی `.br`/`.gz` به هیچ `IfModule` وابسته نیست؛
  از `mod_rewrite` و `mod_headers` استفاده می‌کند که LiteSpeed کامل
  پشتیبانی می‌کند. **همان بخش است که بیشترین اثر را دارد.**

### چیزی که روی سایت زنده تست کردم و باز بود

| مسیر | نتیجه |
|---|---|
| `/.env` | ۴۰۳ ✅ |
| `/app/Config/Database.php` | ۴۰۳ ✅ |
| `/vendor/autoload.php` | ۴۰۳ ✅ |
| `/composer.json` | ۴۰۳ ✅ |
| **`/spark`** | **۲۰۰ — سورس PHP خام** ❌ |
| **`/LICENSE`** | **۲۰۰** ❌ |
| **`/README.md`** | **۲۰۰** ❌ |
| **`/builds`** | **۲۰۰** ❌ |
| **`/default.php`** | **۲۰۰** ❌ |

هیچ‌کدام رمز نیستند، ولی نشان می‌دهند **لیست سیاه فقط چیزهایی را
می‌بندد که کسی یادش بوده اضافه کند**. `spark` پسوند ندارد، پس از فیلتر
رد می‌شد و سورس‌اش به‌صورت متن سرو می‌شد.

قاعده‌ی جدید در `.htaccess` **هر فایل بدون پسوند** را می‌بندد، که کل این
دسته را یک‌جا حل می‌کند. `index.php` پسوند دارد، پس دست‌نخورده می‌ماند.

`default.php` صفحه‌ی پیش‌فرض هاستینگر است — **پاکش کن**، بستن کافی نیست.

### ساخت کلید رمزنگاری بدون SSH

پلن‌های اشتراکی هاستینگر همیشه SSH ندارند. برای همین `tools/genkey.php`
را گذاشتم:

1. با File Manager در hPanel آپلودش کن به ریشه‌ی سایت
2. یک‌بار `https://panel.zeromods.id/genkey.php` را باز کن
3. خطی که چاپ می‌کند را در `.env` جای `encryption.key` بگذار
4. **همان لحظه `genkey.php` و `.genkey-used` را پاک کن**

اگر یادت رفت پاک کنی خطر کمتری دارد: بار دوم اجرا نمی‌شود و خطای ۴۱۰
می‌دهد. ولی باز هم پاکش کن — این یک راه اضطراری است، نه چیزی که باید
روی سرور بماند.

### تغییر Document Root در hPanel

الان هم `/` و هم `/public/` سایت را نشان می‌دهند — یعنی ریشه‌ی وب روی
پوشه‌ی اصلی پروژه است. در hPanel می‌شود ریشه‌ی دامنه را به زیرپوشه
تغییر داد. بعدش:
...
**قبلش از کل سایت بکاپ بگیر.** اگر مسیر اشتباه شود سایت موقتاً بالا
نمی‌آید و باید از hPanel برگردانی.

### تنظیمات hPanel که ارزش چک‌کردن دارند

- **PHP version**: روی ۸.۱ یا بالاتر باشد
- **Optimization / LiteSpeed Cache**: فشرده‌سازی روشن
- **Cron Jobs**: برای پاک‌سازی دوره‌ای جدول لاگ ورود
